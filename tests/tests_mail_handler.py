...
class MailHandlerTest(TestCase):
    ...
    def test_send(self):
        ...
        self.assertEqual(mail.outbox[0].subject, 'Subject first string and second string')
        self.assertEqual(mail.outbox[0].from_email, 'from@example.com')
        self.assertEqual(mail.outbox[0].to, ['to@example.com'])

    # _send()
    def test__send(self):
        msg = mail.EmailMultiAlternatives("subject", "body", "from@example.com", ["to@example.com", ])
    ...
    def test__validate_values(self):
        ...
        with self.assertRaisesMessage(TypeError, "TemplateMail.from_address needs to be a string."):
            mailhandler._validate_values()

        mailhandler.from_address = ""
        with self.assertRaisesMessage(ValidationError, 'Introdueix una adreça de correu electrònic vàlida'):
            mailhandler._validate_values()
    ...
    def test__setup_email(self):
        ...
        # EmailMultiAlternatives appends HTML content into msg.alternatives list.
        html_content = [
    ...
    def test__get_bcc_with_debugging_copy(self):
        ...
        self.assertListEqual(mailhandler._get_bcc_with_debugging_copy(), compare_to)
        ...
        compare_to = ["bcc@example.com", "debug1@example.com", "debug2@example.com", ]
        with self.settings(DEBUG=True):
            self.assertListEqual(mailhandler._get_bcc_with_debugging_copy(), compare_to)

    # _normalize_to_iterable()
    def test__normalize_to_iterable(self):
        with self.assertRaisesMessage(TypeError, "Value '10 should be a string or an iterable."):
